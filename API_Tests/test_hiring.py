...
def test_TC1_Login_Get_Request(setup_jwt_token):
    ...
    assert "hiring_manager" in response.text, "Expected content not found in response"

def test_TC2_Get_Job_Request(setup_jwt_token):
    url = Base_URL + "/api/jobs"
    response = requests.get(url, headers=headers)
    assert response.status_code == 200, f"Failed to get job request. Status code: {response.status_code}"
    print("Respone:",response.text)
    assert "default_org_logo.png" in response.text, "Expected content not found in response"
